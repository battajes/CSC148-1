    ...
    def is_heap(self, index=1):
        ...
        if self.is_empty():
            ...
            if len(self.items) == right:
                if index_item >= self.items[left]: 
                    return True               
                else:
                    return False
            else: 
                if index_item < self.items[right]:
                    return False
                elif index_item < self.items[left]:
                    return False
                elif index_item >= self.items[right] and index_item >= self.items[left]:
                    return True
                else:
                    return self.is_heap()
        ...
        #elif index * 2 + 1 < len(self.items) - 1:
            ## index node has two children
            #left_item = self.items[left]
            #right_item = self.items[index * 2 + 1] 
            #if index_item > right_item >= left_item:
                #return True
            #else:
                #return False
        #elif left < len(self.items):
...
def heapsort(lst):
    ...
    if len(heap.items) > 1: 
        i = 0
        while i != len(heap.items): 
            heap.bubble_down(i)
            i = i +1
        for j in heap.items:
            print ('o')
            try:
                if heap.items[j] > heap_items[j+1]:
                        heap.swap(j,j+1)
                        print('ho')
                else:
                    print('oh')
